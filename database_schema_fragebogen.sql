-- ============================================================================
-- FRAGEBOGEN SYSTEM DATABASE SCHEMA (v2 - canonical)
-- ============================================================================
-- Hierarchical structure:
-- Questions (Plants) -> Modules (Bouquets) -> Fragebogen (Packaging)
--
-- Answer model:
--   fb_responses     - one row per questionnaire run / market visit
--   fb_response_answers - one row per answered question in that run
--
-- Option / matrix format (v2):
--   options JSONB = [{"id": "opt_<12hex>", "label": "Display Text"}, ...]
--   matrix_config JSONB = {
--     "rows":    [{"id": "row_<12hex>", "label": "Row Label"}, ...],
--     "columns": [{"id": "col_<12hex>", "label": "Col Label"}, ...]
--   }
--   Answers reference these IDs, labels are resolved from the current definition.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS fb_questions (
    ...
    --   [{"id": "opt_<12hex>", "label": "Option Text"}, ...]
    options JSONB,
    ...
    -- Images attached to the question (array of public URLs)
...
-- ============================================================================
-- 4. MODULE RULES TABLE (fb_module_rules)
-- Conditional logic stored at the module level.
--
-- trigger_answer stores MACHINE VALUES (not display labels):
--   - yesno:          'true' or 'false'
--   - single_choice:  option id   (e.g. 'opt_abc123def456')
--   - multiple_choice: option id  (same)
--   - matrix:         'rowId: colId'
--   - likert/numeric/slider: numeric string
-- ============================================================================
...
-- ============================================================================
-- 6. FRAGEBOGEN-MODULES JUNCTION (fb_fragebogen_modules)
-- ============================================================================
...
-- ============================================================================
-- 7. FRAGEBOGEN-MARKETS JUNCTION (fb_fragebogen_markets)
-- ============================================================================
...
-- ============================================================================
-- 8. ZEITERFASSUNG SUBMISSIONS TABLE (fb_zeiterfassung_submissions)
-- Time tracking data per GL per market visit
-- ============================================================================
CREATE TABLE IF NOT EXISTS fb_zeiterfassung_submissions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    response_id UUID REFERENCES fb_responses(id) ON DELETE SET NULL,
    fragebogen_id UUID REFERENCES fb_fragebogen(id) ON DELETE SET NULL,
    gebietsleiter_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    market_id VARCHAR(50) NOT NULL REFERENCES markets(id) ON DELETE CASCADE,
    fahrzeit_von TIME,
    fahrzeit_bis TIME,
    fahrzeit_diff INTERVAL,
    besuchszeit_von TIME,
    besuchszeit_bis TIME,
    besuchszeit_diff INTERVAL,
    distanz_km DECIMAL(10, 2),
    kommentar TEXT,
    food_prozent INTEGER CHECK (food_prozent >= 0 AND food_prozent <= 100),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),

    CONSTRAINT unique_zeiterfassung_per_response UNIQUE (response_id)
);

CREATE INDEX IF NOT EXISTS idx_fb_zeiterfassung_response ON fb_zeiterfassung_submissions(response_id);
CREATE INDEX IF NOT EXISTS idx_fb_zeiterfassung_fragebogen ON fb_zeiterfassung_submissions(fragebogen_id);
CREATE INDEX IF NOT EXISTS idx_fb_zeiterfassung_gl ON fb_zeiterfassung_submissions(gebietsleiter_id);
CREATE INDEX IF NOT EXISTS idx_fb_zeiterfassung_market ON fb_zeiterfassung_submissions(market_id);
CREATE INDEX IF NOT EXISTS idx_fb_zeiterfassung_created ON fb_zeiterfassung_submissions(created_at DESC);

-- ============================================================================
-- 9. RESPONSES TABLE (fb_responses)
-- One row per questionnaire run per market visit.
-- NOTE: Multiple runs per (fragebogen, GL, market) are intentionally allowed.
-- ============================================================================
CREATE TABLE IF NOT EXISTS fb_responses (
    ...
    -- Optional link to the concrete market visit that triggered this run
    zeiterfassung_submission_id UUID REFERENCES fb_zeiterfassung_submissions(id) ON DELETE SET NULL,
    ...
    -- No unique constraint — multiple runs are supported
);
...
-- History lookup index for the future answer review UI
CREATE INDEX IF NOT EXISTS idx_fb_responses_history
    ON fb_responses(fragebogen_id, market_id, gebietsleiter_id, started_at DESC);

-- ============================================================================
-- 10. RESPONSE ANSWERS TABLE (fb_response_answers)
-- One row per answered question within a response run.
--
-- answer_kind indicates which payload column holds the actual value:
--   text    -> answer_text     (open_text, single_choice optionId, barcode_scanner)
--   numeric -> answer_numeric  (open_numeric, slider, likert)
...
CREATE TABLE IF NOT EXISTS fb_response_answers (
    ...
    -- Denormalized type - avoids joining fb_questions for analytics/stats queries
    question_type VARCHAR(30),

    -- Which payload column is populated
    answer_kind VARCHAR(20) CHECK (answer_kind IN ('text', 'numeric', 'boolean', 'json', 'file')),

    -- Payload columns — exactly one should be non-null for a saved answer
    answer_text    TEXT,              -- open_text, single_choice (optionId), barcode_scanner
    answer_numeric DECIMAL(15, 4),    -- open_numeric, slider, likert
    answer_boolean BOOLEAN,           -- yesno (true=Ja, false=Nein)
    answer_json    JSONB,             -- multiple_choice (string[]), matrix ({rowId: colId})
    answer_file_url TEXT,             -- photo_upload
    ...
    -- One answer per question per response run
    -- CONSTRAINT unique_response_answer UNIQUE (response_id, question_id, module_id)
    -- NOTE: constraint removed to allow re-answering (upsert by application logic)
);
...
CREATE INDEX IF NOT EXISTS idx_fb_response_answers_kind ON fb_response_answers(question_type, answer_kind);
...
CREATE TRIGGER update_fb_response_answers_updated_at
    BEFORE UPDATE ON fb_response_answers
    FOR EACH ROW EXECUTE FUNCTION update_fb_updated_at_column();

-- ============================================================================
-- AUTO-UPDATE FRAGEBOGEN STATUS
-- ============================================================================
...
SELECT
    m.id, m.name, m.description, m.archived, m.created_at, m.updated_at,
...
SELECT
    f.id, f.name, f.description, f.start_date, f.end_date, f.status,
    f.archived, f.zeiterfassung_active, f.created_at, f.updated_at,
...
SELECT
    q.id, q.type, q.question_text, q.is_template, q.archived, q.created_at,
...
ALTER TABLE fb_zeiterfassung_submissions ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Admins full access on fb_fragebogen_markets" ON fb_fragebogen_markets
    FOR ALL USING (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role = 'admin'));
CREATE POLICY "Admins full access on fb_zeiterfassung_submissions" ON fb_zeiterfassung_submissions
    FOR ALL USING (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role = 'admin'));
...
CREATE POLICY "GLs can view modules in assigned fragebogen" ON fb_modules
    FOR SELECT USING (
        EXISTS (
            ...
            AND f.status = 'active' AND f.archived = false
            AND m.gebietsleiter_id = auth.uid()::text
        )
    );
...
CREATE POLICY "GLs can manage own zeiterfassung_submissions" ON fb_zeiterfassung_submissions
...
CREATE POLICY "GLs can manage own response_answers" ON fb_response_answers
    FOR ALL USING (
        EXISTS (SELECT 1 FROM fb_responses r
